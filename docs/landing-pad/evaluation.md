...
=== "Height"
    ...
    The limitation of the method is also visible: the surroundings shrink with the pad
    and are replaced by flat floor colour. A real photo taken from higher up would show more
...
!!! tip "Time separates them better than confidence does"
    The false detections occurred singly; the correct ones persisted. Requiring the pad
    to appear in three of four consecutive frames in approximately the same position
    removes the noise entirely. A landing controller should not act on a single frame in
    any case.
