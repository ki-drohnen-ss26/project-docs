...
Augmentation
: Generating additional training pictures by altering existing ones: flipping,
rotating, blurring, darkening. It teaches the model that a pad is still a pad when it looks
slightly different.
...
Threshold
: The confidence below which a detection is discarded. Set too low it admits noise;
set too high it misses real pads.
...
:
    - mAP50 — *was it found?* A detection counts as correct if the rectangle
      overlaps the real pad by at least half.
    - mAP50-95 — *was it found, and tightly enclosed?* Considerably stricter, so
      this figure is always lower. Our model scores 0.995 on the first and 0.81 on the
...
Saturated metric
: A score that no longer discriminates because every candidate reaches close to the
