...
Turning that into *"the pad is 1.2 m to the right"*, which is what the flight software
needs, is a separate step that requires the drone's height. That step is written but has
not run yet; see [Talking to the flight software](integration.md).

That is the whole job. The remainder of this section documents how that answer was made
reliable.
...
Three values from the flight software determine the requirements on the detector:
...
!!! note "Terminology"
    Terms such as *recall*, *quantisation* and *false positive* are defined in the
...
!!! note "Distance is less of a constraint than the measurements suggest"
...
!!! info "Status (2026-09-22)"
    ...
    All measurements reported in this section were obtained from photographs, not from
    flight data.
...
The pages are ordered to be read in sequence. Each begins with a summary of its
contents.
