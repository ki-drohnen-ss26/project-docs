...
checks on, correct Li-Ion battery failsafe) and pins the team's chosen `EK3_SRC1_POSZ = 2`
together with the protocol parameters (`RNGFND1_GNDCLEAR = 5`, the lowest value Mission
Planner accepts for this parameter on the real aircraft; a `2` matching the true ~2 cm
mounting height was tried on 2026-09-21 and refused). Load the baseline, then
...
| EKF height source | `EK3_SRC1_POSZ = 2` (rangefinder) is the configuration that currently flies, not one the assignment prescribes: the task asks the project to *use* the LiDAR and the optical flow for altitude and position hold, and it says nothing about which EKF source parameter carries the vertical position. This crash is the documented failure mode of that choice, so we fly it under a safety protocol: `preflight.py` measures EKF-altitude drift on the ground and prints a DO-NOT-FLY verdict (twenty seconds would have caught this before any of the five flights); a bench hand-lift test proves the EKF follows a real lift; the takeover gate and an in-flight cross-check watch the estimate against the raw rangefinder; and `RNGFND1_GNDCLEAR` is set to `5`, the parameter's own minimum in Mission Planner on the real aircraft (a `2` matching the true ~2 cm mounting height was tried and refused). Moving the EKF height source back to the barometer is an option we have deliberately not taken, not one the task rules out: the rangefinder is the sensor the task is about, and the 2026-08-25 SITL work showed the on-ground non-fusion was the `RNGFND1_MIN_CM` validity floor rather than the source choice itself. It stays on the table if the real aircraft disagrees, and a parameter diff against a team flying the same sensor with `POSZ = 2` is still pending. For as long as we fly this setting the barometer stays logged as an independent witness rather than as the EKF source. |
