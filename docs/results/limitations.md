...
| **The LiDAR must never be the sole EKF height source.** On the ground it correctly reads its own mounting height (~0.02 m) — but with `EK3_SRC1_POSZ=2` EKF3 never fused a height at all. | The vertical estimate diverged quadratically to **−1070 m while parked on the floor**; every altitude-controlled mode then chases that estimate. Mitigation: `EK3_SRC1_POSZ=1` (baro primary), rangefinder for low-altitude terrain following only; `preflight.py` measures ground-side EKF drift and prints a DO-NOT-FLY verdict. |
...
| Limit | Consequence / mitigation |
|---|---|
| **The CPU cannot run YOLO inference alongside MAVLink handling.** The trained pad detector as `pad_320_int8.tflite` would have to run on the Pi's CPU — a Zero 2 W cannot sustain that next to the mission loop (the CPU path needs `SKIP_FRAMES = 2` to keep up). | Detection must run *on the camera sensor* (Sony IMX500), which loads **only** `.rpk` packages. That re-export is **done** as of 2026-08-19 and its quantisation loss is measured (mAP50 and recall unchanged); what remains is deploying it and verifying the decoded offsets on the bench — see [Landing Pad Detection](../landing-pad/deployment.md). |
| **The IMX500 holds exactly one network on the sensor**, and swapping costs a camera restart. | The Pi CPU path could run a pad detector *and* a person detector per frame; the AI Camera cannot. Detecting people on the pad as well would require a single two-class model, which measurably costs pad recall in the hard cases — [measured here](../landing-pad/training.md#one-model-or-two). |
